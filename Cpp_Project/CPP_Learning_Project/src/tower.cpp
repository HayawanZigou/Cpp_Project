...
WaypointQueue Tower::get_instructions(Aircraft& aircraft)
{
    if (!aircraft.is_at_terminal)
    {
        ...
        if (aircraft.distance_to(airport.pos) < 5)
        {
            ...
            if (!vp.first.empty())
            {
                reserved_terminals.emplace(&aircraft, vp.second);// TASK_0 C-6: remplacement de emplace_back par emplace.
                return vp.first;
            }
            else
            {
    ...
    else
    {
        ...
        // TASK-0 C-6: plus besoin de la fonction "find_craft_and_terminal"
        // que l'on remplace par "find".
        ...
        if (!terminal.is_servicing())
        {
            ...
            aircraft.service_done = true; // TASK-0 C-4: service effectué donc on peut effacer l'avion.
